...
Nel caso in cui le grandezze siano assegnate nel file dato, verificare i valori e stimare l’errore.
...
**C)** Per il nuovo valore del dislocamento $\nabla_\text{N}$ si determini la tabella delle semi-larghezze al piano di galleggiamento (si prendano 20 ordinate). Si calcoli quindi, utilizzando il metodo di integrazione i) ai trapezi e ii) di Simpson, le seguenti quantità (si può sviluppare un algoritmo in Matlab o Python):
- Area di galleggiamento
- $\text{BM}$
...
```math
0.3 \cdot M_0 (1 + sin(2\Phi)),
```

essendo $M_0$ il momento statico in `G)`. Si determini l’angolo di sbandamento dinamico, partendo da un angolo iniziale di 0° e di -10°.


**I)** Si consideri ora che la nave, al dislocamento considerato, abbia un raggio di inerzia
pari a $r_{xx} = 0.3 \cdot B_{max}$. Si determini il periodo naturale di rollio assumendo un contributo del momento di inerzia aggiunto pari al 20% del contributo di inerzia ordinario della nave.
